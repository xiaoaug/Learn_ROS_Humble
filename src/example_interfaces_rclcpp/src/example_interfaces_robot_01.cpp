...
public:
    ...
    ~Robot() = default;
    // 对外提供获取当前状态、获取当前位置和
    // 移动一定的距离三个接口，其中移动指定距离这个函数每移动一步会休眠 500ms
    // 移动指定的距离
    int move_distance(float distance)
    {
        status_ = example_ros2_interfaces::msg::RobotStatus::STATUS_MOVEING;
        target_pose_ += distance;
        // 当目标距离和当前距离大于 0.01 则持续向目标移动
...
private:
    ...
    void timer_callback()
    {
        ...
        RCLCPP_INFO(this->get_logger(), "发布: 状态数据位置为 %f，状态为 %d", message.pose, message.status);
        // 发布消息
