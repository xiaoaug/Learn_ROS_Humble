    ...
    def move_distance(self, distance):
        ...
        while math.fabs(self.target_pose_ - self.current_pose_) > 0.01:
            step = (
                distance
                / math.fabs(distance)
                * math.fabs(self.target_pose_ - self.current_pose_)
                * 0.1
            )  # 计算一步移动距离
            self.current_pose_ += step  # 移动一步
            print(f"移动了：{step:.5f}，当前位置：{self.current_pose_:.5f}，当前状态：{self.status_}")
            sleep(0.5)  # 休息 0.5s
...
class ExampleInterfacesRobot02(Node):
    def __init__(self, name):
        ...
        self.publisher_timer_ = self.create_timer(1.0, self.publisher_timer_callback)
    ...
    def publisher_timer_callback(self):
        ...
        self.get_logger().info(f"发布当前状态：{msg.status}，位置：{msg.pose:.5f}")
